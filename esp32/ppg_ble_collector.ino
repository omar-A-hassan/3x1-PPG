 ...
 * 
 * Collects 1 minute of PPG data at 100 Hz and transmits via BLE to iOS app
 * 
 * Hardware:
 *   - ESP32 DevKit
 ...
 *   MAX30102 SDA  -> ESP32 GPIO21 (default I2C SDA)
 *   MAX30102 SCL  -> ESP32 GPIO22 (default I2C SCL)
 *   
 * Libraries Required:
...
void initBLE() {
  ...
  // Create Data Characteristic (for transmitting PPG data)
  ...
  // Create Control Characteristic (for receiving commands from iOS)
...
void handleCollecting() {
  ...
  if (currentTime - lastSampleTime >= SAMPLE_INTERVAL_MS) {
    ...
    if (!goodQuality) {
      ...
      if (poorQualitySamples > 100) {  // More than 1 second of poor quality
...
void handleTransmitting() {
  ...
  // Transmit data in chunks (BLE MTU limit is typically 512 bytes)
  const int CHUNK_SIZE = 200;  // 200 samples = 400 bytes per packet
  ...
  for (int chunk = 0; chunk < totalChunks; chunk++) {
    ...
    // Send chunk
    uint8_t* dataPtr = (uint8_t*)&ppgBuffer[startIdx];
    int byteCount = chunkSamples * sizeof(uint16_t);
    
    pDataCharacteristic->setValue(dataPtr, byteCount);
    ...
    Serial.printf("Transmitted chunk %d/%d (%.1f%%)\n", chunk + 1, totalChunks, progress);
